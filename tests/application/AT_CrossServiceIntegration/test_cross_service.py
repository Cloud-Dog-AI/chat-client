...
async def test_at_cross_service_integration(env_file: str) -> None:
    ...
    spam_folder = str(
        cfg.get("chat_tests.at1_23.imap_spam_folder")
        or os.environ.get("IMAP_SPAM_FOLDER", "INBOX.Fail2ban")
    )
    ...
    try:
        ...
        async with httpx.AsyncClient(base_url=base_url, headers=api_headers(cfg), timeout=timeout) as client:
            ...
            resolved_spam_folder = spam_folder
            older_messages: list[dict[str, Any]] = []
            recent_messages: list[dict[str, Any]] = []
            for candidate_folder in _candidate_imap_folders(spam_folder):
                ...
                if older_messages or recent_messages:
                    resolved_spam_folder = candidate_folder
                    break
            if not older_messages and not recent_messages:
                pytest.fail(
                    "PREREQUISITE: no messages found in candidate spam folders "
                    f"{_candidate_imap_folders(spam_folder)}"
                )
