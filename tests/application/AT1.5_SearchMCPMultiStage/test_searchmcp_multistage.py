...
@pytest.mark.asyncio
async def test_at1_5_searchmcp_multi_step_and_resume(env_file):
    cfg = ConfigManager(env_file=env_file)
    base_url = api_base_url(cfg)
    timeout_seconds = float(_require_cfg(cfg, "client_api.request_timeout_seconds"))
    marker = str(_require_cfg(cfg, "chat_tests.searchmcp_marker")).strip()
    response_policy = load_response_policy(cfg)

    tool_name = str(_require_cfg(cfg, "mcp.at1_5.search_tool_name")).strip()
    corruption_args = _parse_json_obj(
        _require_cfg(cfg, "mcp.at1_5.corruption_search_args"), "mcp.at1_5.corruption_search_args"
    )
    happiness_args = _parse_json_obj(
        _require_cfg(cfg, "mcp.at1_5.happiness_search_args"), "mcp.at1_5.happiness_search_args"
    )
    news_args = _parse_json_obj(
        _require_cfg(cfg, "mcp.at1_5.news_search_args"), "mcp.at1_5.news_search_args"
    )

    step1_prompt = str(_require_cfg(cfg, "chat_tests.at1_5.step1_prompt"))
    step2_prompt = str(_require_cfg(cfg, "chat_tests.at1_5.step2_prompt"))
    step3_prompt = str(_require_cfg(cfg, "chat_tests.at1_5.step3_prompt"))
    step4_prompt = str(_require_cfg(cfg, "chat_tests.at1_5.step4_prompt"))
    step5_prompt = str(_require_cfg(cfg, "chat_tests.at1_5.step5_prompt"))

    step1_tokens = _parse_json_list(
        _require_cfg(cfg, "chat_tests.at1_5.step1_expected_tokens"),
        "chat_tests.at1_5.step1_expected_tokens",
    )
    corruption_tokens = _parse_json_list(
        _require_cfg(cfg, "chat_tests.at1_5.corruption_search_tokens"),
        "chat_tests.at1_5.corruption_search_tokens",
    )
    happiness_tokens = _parse_json_list(
        _require_cfg(cfg, "chat_tests.at1_5.happiness_search_tokens"),
        "chat_tests.at1_5.happiness_search_tokens",
    )
    news_tokens = _parse_json_list(
        _require_cfg(cfg, "chat_tests.at1_5.news_search_tokens"),
        "chat_tests.at1_5.news_search_tokens",
    )
    corruption_empty_tokens = _parse_json_list(
        _require_cfg(cfg, "chat_tests.at1_5.corruption_empty_tokens"),
        "chat_tests.at1_5.corruption_empty_tokens",
    )
    happiness_empty_tokens = _parse_json_list(
        _require_cfg(cfg, "chat_tests.at1_5.happiness_empty_tokens"),
        "chat_tests.at1_5.happiness_empty_tokens",
    )
    news_empty_tokens = _parse_json_list(
        _require_cfg(cfg, "chat_tests.at1_5.news_empty_tokens"),
        "chat_tests.at1_5.news_empty_tokens",
    )
    corruption_missing_response_tokens = _parse_json_list(
        _require_cfg(cfg, "chat_tests.at1_5.corruption_missing_response_tokens"),
        "chat_tests.at1_5.corruption_missing_response_tokens",
    )
    corruption_missing_response_optional = _parse_json_list(
        _require_cfg(cfg, "chat_tests.at1_5.corruption_missing_response_optional_tokens"),
        "chat_tests.at1_5.corruption_missing_response_optional_tokens",
    )
    happiness_missing_response_tokens = _parse_json_list(
        _require_cfg(cfg, "chat_tests.at1_5.happiness_missing_response_tokens"),
        "chat_tests.at1_5.happiness_missing_response_tokens",
    )
    happiness_missing_response_optional = _parse_json_list(
        _require_cfg(cfg, "chat_tests.at1_5.happiness_missing_response_optional_tokens"),
        "chat_tests.at1_5.happiness_missing_response_optional_tokens",
    )
    news_missing_response_tokens = _parse_json_list(
        _require_cfg(cfg, "chat_tests.at1_5.news_missing_response_tokens"),
        "chat_tests.at1_5.news_missing_response_tokens",
    )
    news_missing_response_optional = _parse_json_list(
        _require_cfg(cfg, "chat_tests.at1_5.news_missing_response_optional_tokens"),
        "chat_tests.at1_5.news_missing_response_optional_tokens",
    )
    compare_tokens = _parse_json_list(
        _require_cfg(cfg, "chat_tests.at1_5.compare_expected_tokens"),
        "chat_tests.at1_5.compare_expected_tokens",
    )
    retry_attempts = int(_require_cfg(cfg, "chat_tests.at1_5.api_retry_attempts"))
    retry_backoff_seconds = float(_require_cfg(cfg, "chat_tests.at1_5.api_retry_backoff_seconds"))

    require_initialize = bool(cfg.get("mcp.at1_5.require_initialize_search") or False)
    search_server_index_raw = cfg.get("mcp.at1_5.search_server_index")
    try:
        search_server_index = int(search_server_index_raw) if search_server_index_raw is not None else 0
    except (TypeError, ValueError) as e:
        raise RuntimeError("CRITICAL ERROR: mcp.at1_5.search_server_index must be an integer") from e
    ...
    try:
        async with httpx.AsyncClient(base_url=base_url, timeout=timeout_seconds) as client:
            ...
            resp = await client.post("/sessions", json={"metadata": {"suite": "at1.5"}}, headers=headers)
            assert resp.status_code == 200
            ...
            if tool_name not in tool_names:
                raise RuntimeError(f"CRITICAL ERROR: Search MCP tool missing: {tool_name}")
            ...
            if marker not in step1:
                raise RuntimeError("CRITICAL ERROR: response missing SEARCHMCP marker")
            ...
            corruption_text = format_tool_output(_extract_tool_text(corruption_result), cfg, search_server_index)
            if not corruption_text:
                raise RuntimeError("CRITICAL ERROR: Search MCP corruption query returned empty text")
            ...
            step2 = await _send_message(
                client,
                session_id,
                f"{step2_prompt}\n\nOfficial data (from search results):\n{corruption_text}",
                cfg,
                response_policy,
                retry_attempts=retry_attempts,
                retry_backoff_seconds=retry_backoff_seconds,
            ...
            happiness_text = format_tool_output(_extract_tool_text(happiness_result), cfg, search_server_index)
            if not happiness_text:
                raise RuntimeError("CRITICAL ERROR: Search MCP happiness query returned empty text")
            ...
            news_missing = _check_tool_output(
                news_text, news_tokens, news_empty_tokens, "news"
            )
    ...
    try:
        async with httpx.AsyncClient(base_url=base_url, timeout=timeout_seconds) as client:
            ...
            resp = await client.post("/sessions", json={"metadata": {"suite": "at1.5-resume"}}, headers=headers)
            assert resp.status_code == 200
            session_id = resp.json().get("session_id")
            assert session_id
